...
// Menu && Id Generos
fetch(
  "https://api.themoviedb.org/3/genre/movie/list?api_key=b502db08defaa68db6de86429c3ab26f&language=pt-BR"
)
  .then((res) => res.json())
  .then((dados) => {
    //console.log(dados)
  ...
  fetch(url)
    ...
    .then((dados) => {
      ...
      if (dados.results.length == 0) {
        ...
        btnPageNext.style.cursor = "pointer";
    ...
    .then((dados) => {
      ...
      if (dados.results.length == 0) {
        ...
        dados.results.forEach((el) => {
          ...
          divRate.innerHTML = `
                    <p><span class='destaque'>${formatDate(
                      el.release_date
                    )}</span></p>
                    <div class="movieRate">
                    <p><span class='destaque'>${el.vote_average}</span></p>
...
arrowRight.onclick = (e) => {
  ...
  moviesBox.forEach((el) => {
    ...
    el.addEventListener("animationend", function (event) {
      sectionContainerBox1.insertAdjacentElement(
        "afterbegin",
        moviesBox[moviesBox.length - 1]
      );
...
arrowRight2.onclick = (e) => {
  ...
  moviesBox.forEach((el) => {
    el.setAttribute("slide-next", "");
    el.addEventListener("animationend", function (event) {
      sectionContainerBox2.insertAdjacentElement(
        "afterbegin",
        moviesBox[moviesBox.length - 1]
      );
      el.removeAttribute("slide-next");
...
function search(pesquisa, pageNumber = 1) {
  ...
  if (pesquisa != "") {
    ...
    cardContainer.innerHTML = "";
    window.location.hash = `pesquisa=${pesquisaFormat(
      pesquisa
    )}&pagina=${pageNumber}`;
...
function info(cards) {
  ...
  cards.forEach((el) => {
    ...
    <div class="infoBox">
      ...
      <div class="boxMain">
            <div class="posterInfo">
               <img src="${poster_path}" width="100%" alt="" title="${
            dados.title
          }">
            </div>
    ...
    `;
          ...
          infoBox.style.backgroundImage = `url(${
            urlImagem + dados.backdrop_path
          })`;
          ...
          const textInfoHeaderDesc = document.querySelector(
            ".textInfoHeaderDesc"
          );
          dados.genres.forEach((el) => {
            if (el.name != null) {
