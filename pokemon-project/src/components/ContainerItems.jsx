...
export default class ContainerItems extends Component {
  ...
  componentDidMount() {
    ...
    this.loadAllPokemons = fetch("https://pokeapi.co/api/v2/pokemon")
      ...
      .then((dados) => {
        ...
        this.state.btn.push(
          <button
            onClick={this.loadPages}
            className="btn-more"
            url={dados.next}
          >
            <img src={pokebalRedIcon} alt="icon pokebola" />
            ...
            .then((dados) => {
              this.state.pokemonData.push(
                <Cards
                  ...
                  imgType={this.iconPokemons(dados.types[0].type.name)}
                  id={`#${dados.id}`}
                  image={
  ...
  iconPokemons(type) {
    const icon = [
      { name: "normal", icon: iconNormal },
      { name: "fighting", icon: iconFighting },
      { name: "flying", icon: iconFlying },
      { name: "poison", icon: iconPoison },
      { name: "ground", icon: iconGround },
      { name: "rock", icon: iconRock },
      { name: "bug", icon: iconBug },
      { name: "ghost", icon: iconGhost },
      { name: "steel", icon: iconSteel },
      { name: "fire", icon: iconFire },
      { name: "water", icon: iconWater },
      { name: "grass", icon: iconGrass },
      { name: "electric", icon: iconEletric },
      { name: "psychic", icon: iconPhyschic },
      { name: "ice", icon: iconIce },
      { name: "dragon", icon: iconDragon },
      { name: "dark", icon: iconDark },
      { name: "fairy", icon: iconFairy },
      { name: "unknown", icon: iconUnknown },
      { name: "shadow", icon: iconShadow },
    ];
            ...
            .then((data) => {
              this.state.pokemonData.push(
                <Cards
                  ...
                  imgType={this.iconPokemons(data.types[0].type.name)}
                  image={
      ...
      <div className="container-items">
        ...
        {this.state.isMount
          ? createPortal(
              ...
              document.querySelector(".menuMobile-Portal")
            )
          : null}
      </div>
    );
